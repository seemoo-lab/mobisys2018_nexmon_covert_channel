...
#if 1
int32
my_u32_gen_samples(struct phy_info *pi, int f_kHz, unsigned short max_val, unsigned short num_samps, uint32 *tone_buf, int32 theta)
{
    uint8 phy_bw;
    uint16 t;
    int32 rot;
    ...
    for (t = 0; t < num_samps; t++) {
        /* compute phasor */
        wlc_phy_cordic(theta, &sample);
        /* update rotation angle */
        theta += rot;
        /* produce sample values for play buffer */
        tone_buf[t] = ((((unsigned int) FLOAT(sample.q * max_val)) & 0xffff) << 16) |
                    (((unsigned int) FLOAT(sample.i * max_val)) & 0xffff);
    }
